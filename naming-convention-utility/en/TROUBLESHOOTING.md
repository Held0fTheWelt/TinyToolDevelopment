...
Review skip examples in the preview dialog or the Workbench skipped-row filter.

## The Workbench Blocks Apply As Stale

The Workbench apply path compares the reviewed plan hash with the current folder/profile/rule plan.

Possible reasons:

- You changed folder scope after scanning.
- You switched profiles after scanning.
- You edited a Project Custom rule after scanning.
- Assets changed, moved, or were renamed after scanning.

Click **Scan** again, review the refreshed target paths, then apply selected rows.

## I Cannot Edit A Built-In Profile

`epic_recommended` and `tiny_legacy` are read-only baselines.

Use **Copy to Project** to replace the configured Project Custom DataAsset with that profile, then edit the Project Custom rules.
...
If you used the Workbench, also review the JSON/Markdown report under `Saved/NamingConventionUtility/Reports/`.
...
- Preview summary.
- Workbench report path, if available.
