...
```text
Bundled UCM profile
  + enabled plugin-local Config/UnrealCapabilityMesh profiles
  + optional project Config/UnrealCapabilityMesh override
        |
        v
...
For the optional TTER integration:

```text
TinyToolExecutionIntegrationBridge startup
  -> FCapabilityMeshService::ReloadSetup
  -> register UCM projection source
  -> TTER registry discovery
  -> projected tools delegate execution back to UCM
```

The bridge corrects startup ordering only. It does not own setup merging, route policy, approval,
or execution.
...
That folder is the optional highest-priority project override. The setup loader also discovers
`Config/UnrealCapabilityMesh/` below every enabled plugin and merges those read-only provider
profiles with UCM's bundled defaults. Provider plugins therefore carry their own route contracts
into any host project without a project-side copy.
...
An orange warning above the catalog explains that self automatic approval activates the broad experimental policy bypass. Matching deny rules, route asset/project-mutation checks, approval requirements, and dry-run requirements are skipped, so enabled routes can change editor state without those safeguards.
...
An explicit **EXPERIMENTAL** override exists for testing/local own-risk workflows: `experimental_self_approval_all_routes` and `experimental_self_approved_route_ids` in `project_profile.json`. The Route Catalog approval column on the Routes page and MCP tool `mesh_configure_experimental_self_approval` both manage those fields. When active, `bExperimentalPolicyBypass=true` causes matched deny rules, route asset/project-mutation checks, approval requirements, and dry-run requirements to be skipped. This deliberately allows a route that those policy gates would otherwise block and is not intended for production use.
...
The plugin is C++ and uses Unreal Build Tool module rules. All UCM modules use `CppStandardVersion.Cpp20` and `BuildSettingsVersion.Latest`. The supported functionality begins with Unreal Engine 5.4.
