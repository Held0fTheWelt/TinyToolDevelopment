...
The editor adds lifecycle and status actions under **Tools -> AI -> Universal MCP**:
...
Runtime blocks from the **Access Activity** tab are temporary editor-session blocks. They do not write to project settings, hide the tool from `tools/list` for the affected token, reject direct calls with `runtime_access_denied`, and are cleared when the server stops.
