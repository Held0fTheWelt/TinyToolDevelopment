...
The `Authenticate...` button opens a dialog with command, instructions, help link, and a `Copy Command` action for manual execution. LLM Store does not store external tool credentials.
