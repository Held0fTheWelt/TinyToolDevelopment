...
Boolean, default `false`. The **Show Connector Debug** checkbox in the panel's **Layout Connectors** section controls the same setting. When enabled, the editor shows a text marker for each independently resolvable connector endpoint. Every label includes the connector index and kind, the `From` or `To` role, the layout id, and the configured distance; its size follows `DebugTextWorldSize`.

Missing layouts and out-of-range distances are skipped rather than displayed at a false position. Toggling or refreshing connector labels is visual-only: it does not rebuild the track, create connector road geometry, change generated road counts, or expand terrain behavior. Disabling the setting removes only connector labels.
...
Optional route records for pit lanes, alternate routes, service roads, and decorative splines. The legacy actor spline remains the `Main` layout facade.

### LayoutConnectors

Passive semantic links between two layout distances. Each connector stores:

- `FromLayoutId` / `FromDistance`
- `ToLayoutId` / `ToDistance`
- `ConnectorKind`

Known kinds are `PitEntry`, `PitExit`, `Link`, and `ServiceAccess`. `ConnectorKind` is still an open `FName`; custom non-empty kinds are allowed and validate as warnings. Empty kind, missing layouts, bad distances, and same-layout near-equal endpoints are connector-scope errors reported by connector validation, but connector errors do not block geometry rebuilds.

Connector rows do not generate transition meshes, do not rebuild automatically, and do not change terrain deformation behavior.
