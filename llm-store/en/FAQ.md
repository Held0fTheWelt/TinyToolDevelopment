...
## Is there a guided first-time setup?

Yes. The **Setup** tab includes a **Quick-Start** card. It can create a complete mock, Ollama, or cloud OpenAI/Anthropic stack (provider, model, and `default` route) in one click, run a connection test, and refresh readiness. It does not overwrite providers you already created or change an existing `default` route you configured yourself.
