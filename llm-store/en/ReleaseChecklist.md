...
```text
<UnrealBuildCommand> PluginProjectEditor Win64 Development -Project=PluginProject.uproject -NoLiveCoding
```

`<UnrealBuildCommand>` is the build command or script used by your local Unreal Engine installation.
...
- [ ] `Tiny Tools -> LLM Store` opens the tab.
