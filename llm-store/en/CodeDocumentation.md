...
`LLMStoreEditor` is editor-only. It registers the `Tiny Tools -> LLM Store` entry, the nomad tab, the plugin icon/style, built-in optional agent adapters, and add-on UI extension slots.
