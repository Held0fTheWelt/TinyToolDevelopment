...
| `LLMStoreEditor` | Editor UI under `Tiny Tools -> LLM Store`. |
...
**First day:** open `Setup` and use the **Quick-Start** card to create a working Mock, Ollama, or cloud stack with a `default` route in one click (keys stay out of JSON).

**Ongoing:**

1. Create or adjust providers and models in `Setup`.
2. Create routes such as `docs.summarize`, `asset.explain`, or `naming.review`.
3. Test the provider, model, and route.
4. Configure route policies and cost rules.
5. Let other plugins or Blueprints call only the route.
6. Analyze usage in the `Costs` tab.
7. Check release readiness in the `Status` and `Governance` tabs.
...
The `Authenticate...` dialog displays the relevant login command and can run or copy it. Credentials remain with the external tool.
