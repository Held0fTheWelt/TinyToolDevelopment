...
Run the commandlet through the Unreal Editor command-line target for your platform:

```text
<UnrealEditorCommand> Project.uproject -run=LLMStoreCIPolicy -Config=Shipping
```

Example with explicit project path:

```text
<UnrealEditorCommand> "D:\Project\Project.uproject" -run=LLMStoreCIPolicy -Config=Shipping -Unattended -NoSplash -NoSound
```

`<UnrealEditorCommand>` is a placeholder for your installed Unreal Editor command-line target or wrapper.
...
```text
<UnrealEditorCommand> Project.uproject -NullRHI -Unattended -NoSplash -NoSound -ExecCmds="Automation RunTests LLMStore; Quit" -TestExit="Automation Test Queue Empty"
```
