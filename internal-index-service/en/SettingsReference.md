...
| Embedding Executor Id | empty | Preferred IIS embedding executor id. Empty uses normal discovery; the optional IIS LLM Store Bridge sets this to `llmstore` while loaded. |
...
| Enable Runtime Control Mutations | `true` | Master gate for the runtime control surface. When off, tune/act/persist console and API verbs are rejected; reads still work. |
| Retain Recent Diagnostics | `false` | When on, IIS keeps a bounded in-memory ring buffer of recent per-query diagnostics for `diagnostics.recent` / console inspection. |
| Recent Diagnostics Max Count | `64` | Maximum diagnostics retained when Retain Recent Diagnostics is on. |
| Estimate Recall By Default | `false` | When on, searches estimate recall@K unless the query explicitly opts out. Adds an exact reference scan (SYN-1). |
