...
Use a Smart Content Diet BuildPlugin package produced for the installed engine version. The package
retains generated binaries and precompile metadata required by installed builds. Do not strip those
files when copying or archiving the plugin.
...
| Release validation output | `Saved/SCDValidation/` when maintainer validation is run |
...
Release validation is a maintainer workflow. Buyer use does not require local
gate scripts, host-project test modules, or repository-specific paths.
